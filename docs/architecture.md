...
`bash` waits for completion for 10 seconds by default. `yield_time_ms` can explicitly change that wait from 0 to 60 seconds. If the command finishes, the call returns its output and exit code; otherwise it returns the output so far, `status: running`, and a random `session_id`. This wait controls only when MCP yields a response and never kills the command.

There is no command lifetime limit unless `timeout_ms` is explicitly supplied. `bash_continue` returns only output not returned by earlier calls and reports the current status and exit code. Each response reads at most 60,000 new bytes and reports `has_more_output` when another read is needed. `bash_stop` terminates the process group with SIGTERM and escalates to SIGKILL after 1.5 seconds.
