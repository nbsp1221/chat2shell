...
export function normalizeWorkspaceIdentity(
  ...
  const internalIds = [
    structuredContent.workspace_id,
    structuredContent.selected_workspace_id,
  ...
  return {
    ...
    structuredContent: {
      ...
      ...(typeof structuredContent.selected_workspace_id === 'string'
...
function toolResult(response: JsonRpcResponse): CallToolResult {
  if (response.error) {
    throw new Error(`CodexPro ${response.error.code}: ${response.error.message}`);
  }
  if (!response.result) {
    throw new Error('CodexPro returned no tool result');
  }
...
class CodexProSession {
  ...
  constructor(sandbox: Sandbox) {
    if (!sandbox.endpoint || !sandbox.authToken) {
      throw new Error('Sandbox is missing its CodexPro connection');
    }
  ...
  async callTool(name: string, args: Record<string, unknown>): Promise<CallToolResult> {
    ...
    try {
      return toolResult(await this.#request('tools/call', { name, arguments: args }));
    } catch (error) {
  ...
  async #initialize(): Promise<void> {
    ...
    if (response.error) {
      throw new Error(`CodexPro initialize failed: ${response.error.message}`);
    }
    if (!this.#sessionId) {
      throw new Error('CodexPro initialize response did not include a session id');
    }
    await fetch(this.#endpoint, {
      method: 'POST',
      headers: this.#headers(),
  ...
  async #request(
    ...
    if (!response.ok) {
      throw new Error(`CodexPro HTTP ${response.status}: ${await response.text()}`);
    }
  ...
  async call(
    ...
    return this.#sandboxes.withReady(ownerId, sandboxId, async (sandbox) => {
      ...
      return normalizeWorkspaceIdentity(
        await session.callTool(toolName, args),
        sandbox.workspaceId,
      );
    });
  }
