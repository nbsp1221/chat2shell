...
const bashOutputSchema = {
  ...
  properties: {
    ...
    status: { type: "string" as const, enum: ["running", "exited"], description: "Whether the process is still running or has exited." },
    exit_code: {
      anyOf: [{ type: "integer" as const }, { type: "null" as const }],
      description: "Process exit code after exit, otherwise null.",
    },
    output: { type: "string" as const, description: "New combined stdout/stderr not returned by an earlier response." },
    has_more_output: {
      type: "boolean" as const,
      description: "Whether more already-buffered output remains. A running process may produce future output even when this is false.",
    },
  },
...
const bashPollTool: Tool = {
  ...
  description: "Wait for new output, process exit, or yield expiry, then return only new combined stdout/stderr and current status. Poll again while status is running or has_more_output is true.",
  inputSchema: {
    type: "object",
    properties: {
      ...
      yield_time_ms: { type: "integer", minimum: 0, maximum: 60_000, default: 10_000, description: "Maximum time to wait for new output or exit." },
    },
