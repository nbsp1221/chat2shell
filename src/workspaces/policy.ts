...
const blockedNames = new Set([
  ".aws", ".azure", ".config", ".docker", ".gnupg", ".kube", ".local", ".secrets", ".ssh",
]);
...
export class HostPathPolicy {
  ...
  resolveAndValidate(requestedPath: string): string {
    const expanded = requestedPath === "~"
      ? os.homedir()
      : requestedPath.startsWith("~/") ? path.join(os.homedir(), requestedPath.slice(2)) : requestedPath;
    ...
    if (!this.#allowedRoots.some((root) => isInside(resolved, root))) {
      throw new Error(`Workspace must be below an allowed host root: ${this.#allowedRoots.join(", ")}`);
    }
    const names = resolved.split(path.sep);
    const blocked = names.find((name) => blockedNames.has(name));
    if (blocked) throw new Error(`Workspace path contains a protected directory: ${blocked}`);
    return resolved;
  }
}
