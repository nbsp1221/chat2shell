...
export class StateDatabase {
  ...
  insertWorkspace(workspace: Workspace): void {
    this.#database.prepare(`INSERT INTO workspaces
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(workspace.id, workspace.ownerId, workspace.kind, workspace.mode, workspace.root, workspace.status, workspace.createdAt, workspace.retainedUntil ?? null);
  }

  updateWorkspaceStatus(id: string, status: Workspace["status"], retainedUntil?: number): void {
    this.#database.prepare("UPDATE workspaces SET status = ?, retained_until = ? WHERE id = ?")
      .run(status, retainedUntil ?? null, id);
  }

  updateWorkspaceLocation(id: string, root: string, status: Workspace["status"], retainedUntil?: number): void {
    this.#database.prepare("UPDATE workspaces SET root = ?, status = ?, retained_until = ? WHERE id = ?")
      .run(root, status, retainedUntil ?? null, id);
  }

  getWorkspace(id: string, ownerId?: string): Workspace | undefined {
    const row = ownerId
      ? this.#database.prepare("SELECT * FROM workspaces WHERE id = ? AND owner_id = ?").get(id, ownerId)
  ...
  findWorkspace(ownerId: string, root: string, mode: Workspace["mode"]): Workspace | undefined {
    const row = this.#database.prepare("SELECT * FROM workspaces WHERE owner_id = ? AND root = ? AND mode = ?")
      .get(ownerId, root, mode);
    return row ? workspaceFromRow(row) : undefined;
  }

  listWorkspaces(ownerId: string): readonly Workspace[] {
    return this.#database.prepare("SELECT * FROM workspaces WHERE owner_id = ? ORDER BY created_at DESC")
      .all(ownerId).map(workspaceFromRow);
  }

  listExpiredRetainedWorkspaces(now: number): readonly Workspace[] {
    return this.#database.prepare("SELECT * FROM workspaces WHERE status = 'retained' AND retained_until <= ?")
      .all(now).map(workspaceFromRow);
  }
  ...
  insertApproval(approval: Approval): void {
    this.#database.prepare(`INSERT INTO approvals
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(approval.id, approval.ownerId, approval.requestedPath, approval.mode, approval.status, approval.workspaceId ?? null, approval.createdAt, approval.decidedAt ?? null);
  }
  ...
  findPendingApproval(ownerId: string, requestedPath: string, mode: Approval["mode"]): Approval | undefined {
    const row = this.#database.prepare("SELECT * FROM approvals WHERE owner_id = ? AND requested_path = ? AND mode = ? AND status = 'pending'")
      .get(ownerId, requestedPath, mode);
    return row ? approvalFromRow(row) : undefined;
  }

  decideApproval(id: string, status: "approved" | "rejected", workspaceId: string | undefined, decidedAt: number): void {
    this.#database.prepare("UPDATE approvals SET status = ?, workspace_id = ?, decided_at = ? WHERE id = ?")
      .run(status, workspaceId ?? null, decidedAt, id);
  }

  listApprovals(status?: Approval["status"]): readonly Approval[] {
    const rows = status
      ? this.#database.prepare("SELECT * FROM approvals WHERE status = ? ORDER BY created_at DESC").all(status)
  ...
  getSandbox(id: string, ownerId?: string): Sandbox | undefined {
    const row = ownerId
      ? this.#database.prepare("SELECT * FROM sandboxes WHERE id = ? AND owner_id = ?").get(id, ownerId)
  ...
  findActiveSandbox(ownerId: string, workspaceId: string): Sandbox | undefined {
    const row = this.#database.prepare(`SELECT * FROM sandboxes
