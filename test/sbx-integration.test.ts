...
import { loadCodexProToolManifest } from "../src/codexpro/tool-manifest.js";
...
test("the public MCP boundary routes full shell and private Docker only into a real microVM", { skip: !enabled }, async () => {
  ...
  const appConfig: AppConfig = {
    ...
    reaperIntervalMs: 60_000, codexProToolMode: "standard",
  };
  ...
  const tools = await loadCodexProToolManifest(appConfig.workspaceRoot, "standard");
  ...
  try {
    ...
    const listed = await callTool(url, 7, "sandbox_list", {});
    assert.equal((listed.structuredContent as { sandboxes: Array<{ id: string }> }).sandboxes[0]?.id, sandboxId);
    const destroyed = await callTool(url, 8, "sandbox_destroy", { sandbox_id: sandboxId });
    ...
    const cloneCreateResult = await callTool(url, 9, "sandbox_create", { workspace_id: cloneWorkspace.id });
    ...
    const cloneWrite = await callTool(url, 10, "write", { sandbox_id: sandboxId, path: "clone-proof.txt", content: "private clone\n" });
    ...
    await callTool(url, 11, "sandbox_destroy", { sandbox_id: sandboxId });
    sandboxId = undefined;
  } finally {
